...
- **Closed-weight-model probe discipline (per ADR-011 reproducibility chain).** Claude Sonnet, GPT-5, Gemini, and other closed-weight commercial models are permitted as one-off *methodological debuggers* (e.g., the 2026-06-01 cross-model probe that isolated Nemotron's failure modes as model-side, not prompt-side). They are **NEVER** part of the paper's comparison stack, KPI tables, ablation runs, or any artifact that lands in `data/runs/` for paper-grade evaluation. The reproducibility chain established by ADR-011 (open weights + open API + two independent reproduction paths) breaks the moment a closed-weight number lands in a results table. Probe data may be documented in `pre_submission_checklist.md` §5.x audit entries (as the §5.1 entry currently does) but does not populate any paper-grade KPI.
...
- 2026-06-01 (ADR-011 + ADR-012 — inference-stack split) — The two-failure-mode evidence accumulated in the post-canonical-IAE smoke (canonical IAE 0.16756 on 5 cycles; completion_tokens P95 = 512 cap-hit on 3/5 cycles; cycle 6 aborted on JSON truncation) plus the cumulative ADR-005-amendment cost on the Mac-Studio MLX stack motivated a strategic pivot to NIM-hosted inference. ADR-011 documents the inference-stack split (NIM-hosted primary, Mac-Studio retained as contingency only) and formalizes the open-weights + open-API reproducibility chain as a paper-methodological asset. ADR-012 supersedes the ADR-005 Qwen ablation specification with DeepSeek-family-on-NIM, restoring cross-family-ablation capability-parity with the primary. §5.1's "model swap" branch is therefore narrowed to *within-NIM* swaps (Nemotron-Ultra-253B and / or DeepSeek upgrade) rather than back to Mac-Studio variants. §6 augmented with the closed-weight-probe-discipline rule (Sonnet et al. are debugging tools, not paper-grade comparison sources).
- 2026-06-01 (first NIM smoke + v1.1 prompt) — First NIM smoke ran with the *original* system prompt against NIM-hosted Nemotron-Super-49B-v1.5 BF16 (single seed 0, output `data/runs/c2_smoke/nominal_baseline/seed0_nim/smoke.json`). Output discipline largely resolved (completion_tokens P95 462 / max 491, no cap hits, no JSON truncation, all 12 cycles completed) — confirms the Mac-Studio cap-hit failure mode was 8-bit-MLX-quantization-specific. Multi-step coherence not resolved and arguably *worse* than Mac-Studio: cycle 0 picks the same (0.995, 0.005) over-purified target as Mac-Studio (zero-IAE bias intact), then cycles 1+ over-correct into (0.980, 0.020) under-purification, settling stably into a wrong-regime attractor by cycle 6. Canonical IAE 1.234 mole-fraction·min over 12 cycles versus 0.168 over 5 cycles on Mac-Studio (~3× worse per cycle). The new failure-mode shape (Run-IAE-driven over-correction) revealed a gap in v1's wording: v1's asymmetric DO-NOT clause covered over-purification but not under-purification. v1.1 (`docs/prompts/2026-06-01_phase3_system_prompt_v1_1.md`) supersedes v1 with a mechanical default rule, a symmetric DO-NOT clause, and a tightened 20-word rationale cap. ADR-005 §5.1 trigger is now *fully* active with two-stack reproducible evidence; the within-NIM escalation ladder (v1.1 on -49B → v1.1 on Ultra-253B → early DeepSeek invocation per ADR-012) is documented in the v1.1 doc.
