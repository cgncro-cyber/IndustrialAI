...
**Status:** **Phase 1 complete** (2026-05-27, Days 9–10 closed). Skogestad Column A dynamic twin per ADR 007: ODE + integrator + steady state, LV + DV + L/D-V/B configurations, finite-difference and CasADi-symbolic linearization (both backends pass the Skogestad 1997 mini-gate invariants), regulatory PIDs + setpoint interface, data-logging contract, mass-balance check, rate-limiter divergence guard, 1080-point operating-window sweep (100 % convergence), walkthrough notebook, assumptions.md with full Skogestad citations. CasADi backend gives exact machine-precision Jacobians to be consumed natively by the Phase 2 Linear MPC baseline (`do-mpc`). 84 pytest cases pass, twin-layer coverage ≥ 96 %. Next: Phase 2 (PID tuning + Linear MPC baselines).
